...
```{r labels, echo=F}
strgth_palette <- c("#008080", "#9fdfbf")
...
```{r submem_models, echo=F, results="asis", fig.height=3, fig.width=4.2}
...
for (this.cluster in power.submem.summary %>% pull(!!sym(cluster_id.power)) %>% unique()) {
  ...
  plt <- mod %>% 
    plot_model(type="pred",
               ...
               show.data=F) +
    scale_color_manual(values=strgth_palette, name="") +
    scale_fill_manual(values=strgth_palette, name="") +
    scale_y_continuous(limits = c(0, 1)) +
    labs(y = "P(remember probe)",
         x = paste(y_axis_power_label, "(z)"),
         title = " ") +
  ...
  submem.summary <- mod.data %>%
    ...
    mutate(power_bin = cut(power,
                           breaks = nbins,
                           labels = F)) %>%
    ungroup() %>%
    group_by(subjectId, power_bin, association_strength) %>%
    summarize(power = mean(power),
              prob.remember = mean(trial_outcome.recognition)) %>%
    ungroup() %>%
    group_by(power_bin, association_strength) %>%
    mutate(power_mean = mean(power)) %>%
    ungroup() %>% 
    mutate(power_mean = as.numeric(power_mean)) %>% 
    summarySEwithin(., 
                    measurevar="prob.remember", 
                    withinvars=c("association_strength",
                                 "power_mean"), 
                    idvar="subjectId") %>% 
    mutate(power_mean = as.numeric(as.character(power_mean)))
  
  pred <- data.frame(ggpredict(mod, terms = c("power[all]",
                                              "association_strength[Strong (4x), Weak (1x)]"))) %>% 
    rename(power_mean = x,
           association_strength = group)
  ...
  plt <- ggplot() +
    ...
    geom_line(data = as.data.frame(pred),
              ...
              linewidth = 1) +
    geom_point(data = submem.summary,
               aes(x = power_mean,
                   y = prob.remember,
                   color = association_strength),
               size = 1.5) +
    geom_errorbar(data = submem.summary,
                  aes(x = power_mean,
                      ymin = prob.remember - se,
                      ymax = prob.remember + se,
                      color = association_strength),
                  width = 0.2) + 
    scale_color_manual(values = strgth_palette, name = "") +
    scale_fill_manual(values = strgth_palette, name = "") +
    scale_y_continuous(limits = c(0, 1)) +
    labs(y = "P(remember probe)",
         x = paste(y_axis_power_label, "(z)", sep=" "),
         title = paste("Cluster", this.cluster, sep = " ")) +
    theme(strip.text.x = element_text(size=12))
  
  print(plt)
  
  showtext_opts(dpi = 300)
  ggsave(paste("submem_figures/submem_", power_name, "_cluster-", this.cluster, "_pred_mem.pdf", sep=""),
         plot=plt,
         width=12,
         height=8,
         dpi=300,
  ...
  showtext_opts(dpi = 300)
  ggsave(paste("submem_figures/submem_", power_name, "_cluster-", this.cluster, ".pdf", sep=""),
         plot=joint.plt,
         width=15,
         height=6,
         dpi=300,
...
```{r rt_submem_models, echo=F, results="asis", fig.height=3, fig.width=3.4}
