...
for (this.comparison in comparisons.of.interest) {
  ...
  if (this.comparison == "Memory strength (Weak v. Strong) \n frontal theta 3 v. posterior alpha 4") {
    ...
    mod <- mod.data %>%
      ...
      lmer(match_response.rt ~ power.alpha * power.theta * association_strength * trial_outcome +
             (power.alpha * power.theta * association_strength | subjectId),
           data = .)
    ...
    plt <- plot_model(mod, terms = c("power.theta", "power.alpha"), type="pred")
    print(plt)
  }
}
```
