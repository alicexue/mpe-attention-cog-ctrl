...
for (i in 1:ncomp) {
  ...
  emm <- emmeans(mod, ~ is_mismatch | association_strength, lmerTest.limit = 10346)
  print(emm)
  tidy(emm, effects = "fixed", conf.int = T, conf.level = 0.95) %>% 
    kable(digits=3, caption=paste("PC", i, "; Emmeans", sep="")) %>% 
    kable_styling() %>% 
    cat()
  emm.pairs <- pairs(emm)
  print(emm.pairs)
...
for (pc in 1:ncomp) {
  ...
  slopes <- emtrends(mod, pairwise ~ association_strength * trial_outcome, var = "component_score", lmerTest.limit = 10346)
...
subs.exclude <- c("MPE514", "MPE527") # insufficient number of trials
...
subj.data.submem <- subj.data.scores %>% 
  ...
  mutate(trial_outcome.recognition = recode(trial_outcome.recognition, 
                                            "Hit" = 1,
                                            "Miss" = 0)) %>% 
  group_by(subjectId, component) %>% 
  mutate(component_score = scale(component_score)) %>% 
  ungroup() %>% 
  mutate(expId = recode(expId, 
                        "imm" = "Experiment 1 (same day)",
                        "del" = "Experiment 2 (48 hrs later)"))
```

### Models

```{r recog_models, echo=F, results="asis", eval=run_models}
recog.glmer.models <- list()
...
for (i in 1:ncomp) {
  ...
  mod <- mod.data %>% 
    glmer(trial_outcome.recognition ~ component_score * association_strength * expId + 
            (component_score * association_strength | subjectId),
          data = .)
  ...
  recog.glmer.models[[i]] <- mod 
}

```

```{r show_recog_mods, echo=F, results="asis", fig.height=3, fig.width=6}

for (i in 1:ncomp) {
  mod <- recog.glmer.models[[i]]
  ...
  if (i == 1) {
    slopes <- emtrends(mod, pairwise ~ association_strength * expId, var = "component_score")
    ...
    tidy(slopes$emtrends, conf.int = T, conf.level = 0.95) %>% 
      kable(caption=paste("slopes: remember ~ ", title, sep=""),
            digits=3) %>% 
      kable_styling() %>% 
      cat()
    ...
    tidy(slopes$contrasts, conf.int = T, conf.level = 0.95) %>% 
      select(contrast, estimate, std.error, df, conf.low, conf.high, statistic, adj.p.value) %>% 
      kable(caption=paste("slope contrasts: remember ~ ", title, sep=""),
            digits=3) %>% 
      kable_styling() %>% 
      cat()
  }
  ...
  submem.summary <- subj.data.submem %>%
    dplyr::filter(component == paste("PC", i, sep="")) %>% 
    group_by(subjectId, association_strength, expId) %>%
    mutate(component_score_bin = cut(component_score,
                                     breaks = nbins,
                                     labels = F)) %>%
    ungroup() %>%
    group_by(subjectId, component_score_bin, association_strength, expId) %>%
    summarize(component_score = mean(component_score),
              prob.remember = mean(trial_outcome.recognition)) %>%
    ungroup() %>%
    group_by(component_score_bin, association_strength, expId) %>%
    mutate(component_score_mean = mean(component_score)) %>%
    ungroup() %>% 
    mutate(component_score_mean = as.numeric(component_score_mean)) %>% 
    summarySEwithin(., 
                    ...
                    betweenvars=c("expId"),
                    idvar="subjectId") %>% 
    mutate(component_score_mean = as.numeric(as.character(component_score_mean)))
  
  pred <- data.frame(ggpredict(mod, terms = c("component_score[all]",
                                   "association_strength[Strong (4x), Weak (1x)]",
                                   "expId"))) %>% 
    rename(component_score_mean = x,
           association_strength = group,
           expId = facet)
  
  plt <- ggplot() +
    geom_ribbon(data = as.data.frame(pred),
                aes(x = component_score_mean, 
                    ymin = conf.low, 
                    ymax = conf.high, 
  ...
  showtext_opts(dpi = 300)
  ggsave(paste("submem_figures/submem_PC", i, ".pdf", sep=""),
         plot=plt,
         width=17,
         height=8,
         dpi=300,
...
rt_data.submem <- subj.data.submem %>%
  dplyr::filter(trial_outcome.recognition == 1) %>% 
