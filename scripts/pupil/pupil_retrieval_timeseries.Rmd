...
```{r submem_models, echo=F, results="asis", eval=T}
...
for (this.cluster in pupil.cluster.submem %>% pull(cluster_id) %>% unique()) {
  mod.data <- pupil.cluster.submem %>% 
    ...
    mutate(trial_outcome.recognition = recode(trial_outcome.recognition, 
                                              ...
                                              "Miss" = 0)) %>% 
    group_by(subjectId) %>% 
    mutate(pupil_diameter_z = scale(pupil_diameter_z)) %>% 
    ungroup()
  
  mod <- mod.data %>% 
    glmer(trial_outcome.recognition ~ pupil_diameter_z * association_strength * expId + 
  ...
  mod <- mod.data %>% 
    lmer(recognition_response.rt ~ pupil_diameter_z * association_strength * expId +
           (pupil_diameter_z * association_strength | subjectId),
         data = .)
...
for (this.cluster in pupil.cluster.submem %>% pull(cluster_id) %>% unique()) {
  ...
  submem.summary <- mod.data %>%
    ...
    mutate(pupil_bin = cut(pupil_diameter_z,
                           breaks = nbins,
                           labels = F)) %>%
    ungroup() %>%
    group_by(subjectId, pupil_bin, association_strength, expId) %>%
    summarize(pupil_diameter_z = mean(pupil_diameter_z),
              prob.remember = mean(trial_outcome.recognition == "Hit")) %>%
    ungroup() %>%
    group_by(pupil_bin, association_strength, expId) %>%
    mutate(pupil_mean = mean(pupil_diameter_z)) %>%
    ungroup() %>% 
    mutate(pupil_mean = as.numeric(pupil_mean)) %>% 
    summarySEwithin(., 
                    ...
                    betweenvars=c("expId"),
                    idvar="subjectId") %>% 
    mutate(pupil_mean = as.numeric(as.character(pupil_mean)))
  
  pred <- data.frame(ggpredict(mod, terms = c("pupil_diameter_z[all]",
  ...
  ggsave(paste("submem_figures/submem_cluster_", this.cluster, ".pdf", sep=""),
         plot=cluster.plt,
         width=17,
         height=8,
         dpi=300,
