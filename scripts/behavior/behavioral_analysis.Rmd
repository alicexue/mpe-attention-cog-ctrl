...
kable(data_recog %>%
        ...
        mutate(no_response = is.na(rt)) %>%
        group_by(subjectId, expId, association_strength) %>%
        summarize(n_hit = sum(is_hit, na.rm=T) / 
                  ...
                  n_cr = sum(is_correct_rejection, na.rm=T) / 
                    (sum(is_false_alarm, na.rm=T) + 
                       sum(is_correct_rejection, na.rm=T)) * 100) %>%
        ungroup() %>% 
        group_by(expId, association_strength) %>%
        ...
        mutate(across(everything(), ~replace(., . == "(NA)", " "))) %>% 
        arrange(desc(expId), match(association_strength, c("Strong (4x)", "Weak (1x)", "Familiar item", "Novel"))) %>% 
        select(expId, association_strength, hit, sd_hit, fa, sd_fa, 
               miss, sd_miss, cr, sd_cr),
      col.names = c("expId", "association_strength", "% hit", "(sd)", "% fa", "(sd)", 
                    "% miss", "(sd)", "% cr", "(sd)")) %>%
...
emm <- emmeans(mod, ~ association_strength | expId, lmerTest.limit = 10346)
emm.pairs <- pairs(emm)
print(emm.pairs)
...
emm <- emmeans(ret.rt.mod, ~ is_mismatch | association_strength | incorrect.ret, lmerTest.limit = 10346)
emm.pairs <- pairs(emm)
print(emm.pairs)
tidy(emm.pairs, effects = "fixed", conf.int = T, conf.level = 0.95) %>% 
...
emm <- emmeans(ret.rt.mod, ~ is_mismatch | association_strength | incorrect.ret | expId, lmerTest.limit = 10346)
emm.pairs <- pairs(emm)
print(emm.pairs)
tidy(emm.pairs, effects = "fixed", conf.int = T, conf.level = 0.95) %>% 
...
emm <- emmeans(recog.rt.mod, ~ association_strength | incorrect.recog | expId, lmerTest.limit = 10346)
emm.pairs <- pairs(emm)
tidy(emm.pairs, effects = "fixed", conf.int = T, conf.level = 0.95) %>% 
